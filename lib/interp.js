...
function env(parent,interp){
	...
	// simple operators
	for (var op in { '+':'+', '-':'-', '*':'*', '/':'/', '%':'%' }){
		scope[op] = (function(interp){ 
			var reducefn = 'function(a,b){ return a'+op+'b }';
			return arguments[1] ? interp(arguments,this).reduce(eval(reducefn)) : interp(arguments[0],this) 
		})(interp);
	}	
	// builtin
